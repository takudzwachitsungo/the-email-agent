...
| 04 | [Data Model](04-data-model.md) | SQLite schema, the approval state machine, the three-layer memory model and its upgrade path |
| 05 | [Roadmap](05-roadmap.md) | Phased delivery: core loop → approval → memory & autonomy |

## One-paragraph summary

The system is a **linear pipeline of swappable stages** — not a framework-driven
agent graph — built around two firm rules: **nothing is sent without the owner's
approval (early on)**, and **email is untrusted input** (its content is never
mixed with instructions). It runs as a single Python process against one Gmail
inbox, stores all state and memory in a single SQLite file, and talks to its
language model through one thin provider seam so no vendor is hardcoded. It is
designed to run cheaply (free tiers), survive unattended operation for days, and
grow — one earned, revocable step at a time — from co-pilot toward autopilot.
